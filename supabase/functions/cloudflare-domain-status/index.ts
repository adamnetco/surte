...
Deno.serve(async (req) => {
  ...
  await svc.from("tenant_domains").update({
    ...
    cname_target: fallbackHostname,
    last_checked_at: new Date().toISOString(),
    verified_at: status === "active" ? new Date().toISOString() : null,
  }).eq("hostname", hostname);

  return jsonResponse({ ok: true, hostname, status, ssl_status, ssl_validation_records, cname_target: fallbackHostname, result: cfJson.result });
});
