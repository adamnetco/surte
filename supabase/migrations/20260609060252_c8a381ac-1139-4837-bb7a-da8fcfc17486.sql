...
DROP POLICY IF EXISTS "superadmin updates settings" ON public.auth_settings;
...
DROP POLICY IF EXISTS "user manages own factors" ON public.auth_factors;
...
DROP POLICY IF EXISTS "user manages own passkeys" ON public.auth_webauthn_credentials;
CREATE POLICY "user manages own passkeys" ON public.auth_webauthn_credentials FOR ALL TO authenticated
  USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

-- 4. auth_recovery_codes
...
DROP POLICY IF EXISTS "user sees own recovery rows" ON public.auth_recovery_codes;
...
DROP POLICY IF EXISTS "user sees own login events" ON public.auth_login_events;
...
DROP POLICY IF EXISTS "superadmin sees allowlist" ON public.auth_superadmin_allowlist;
...
DROP POLICY IF EXISTS "superadmin sees break-glass" ON public.auth_break_glass_requests;
...
-- 8. helper: app_current_role (evita colisión con built-in current_role)
