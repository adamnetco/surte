...
Archivos escaneados: **424**  ·  Hallazgos: **69**  
(high: 58, medium: 7, low: 4)
...
- **10** — `src/modules/admin-cms/components/ProductsTab.tsx`
- **10** — `src/modules/admin-cms/components/ShippingTab.tsx`
- **10** — `src/modules/admin-cms/pages/AdminDashboard.tsx`
- **10** — `src/modules/offline/lib/outbox.ts`
- **10** — `src/modules/storefront/pages/ProductoDetalle.tsx`
...
### `src/modules/admin-cms/components/ProductsTab.tsx` (2)
- L64 · insert() without organization_id · `product_media`
  ```supabase.from("product_media").insert({ product_id: productId, media_type: "image", media_url: url, sort_order: currentMax + i, })```
- L147 · select() without organization_id filter · `featured_sections`
  ```supabase.from("featured_sections").select("*").order("sort_order")```

### `src/modules/admin-cms/components/ShippingTab.tsx` (2)
- L16 · select() without organization_id filter · `municipality_settings`
  ```supabase.from("municipality_settings").select("city").eq("is_active", true).order("city")```
...
- L208 · select() without organization_id filter · `app_settings`
  ```supabase.from("app_settings").select("*")```

### `src/modules/offline/lib/outbox.ts` (2)
- L131 · insert() without organization_id · `pos_order_items`
  ```supabase.from("pos_order_items").insert(lines)```
- L140 · insert() without organization_id · `pos_payments`
  ```supabase.from("pos_payments").insert(pays)```

### `src/modules/storefront/pages/ProductoDetalle.tsx` (2)
- L71 · select() without organization_id filter · `product_media`
  ```supabase.from("product_media").select("*").eq("product_id", productId!).order("sort_order")```
- L81 · select() without organization_id filter · `product_presentations`
...
- L49 · insert() without organization_id · `google_reviews`
...
### `src/modules/admin-cms/components/MunicipalitiesTab.tsx` (1)
- L99 · insert() without organization_id · `municipality_settings`
  ```supabase.from("municipality_settings").insert(payload)```
...
### `src/modules/admin-cms/components/SeoContentTab.tsx` (1)
- L73 · insert() without organization_id · `seo_content`
