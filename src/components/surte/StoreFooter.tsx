...
const SocialIcon = ({ type, url }: { type: string; url: string }) => {
  ...
  if (!d) return null;
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-primary-foreground/10 flex items-center justify-center hover:bg-primary-foreground/20 transition-colors">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {navLinks.map((link) => (
                <li key={link.path}>
                  <button onClick={() => navigate(link.path)} className="text-sm opacity-70 hover:opacity-100 transition-opacity">
                    {link.label}
                  </button>
                </li>
              ))}
            ...
            <ul className="space-y-2.5">
              {whatsapp && (
                <li className="flex items-center gap-2 text-sm opacity-80">
                  ...
                  <a href={`https://wa.me/${whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="hover:opacity-100 transition-opacity">
                    {whatsapp}
                  </a>
                </li>
              )}
        ...
        <div className="mt-8 space-y-3">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <a
              href={`${googleMapsUrl}?action=review`}
              target="_blank"
              rel="noopener noreferrer"
