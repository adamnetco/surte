...
const Ayuda = () => {
  ...
  const { whatsapp, email, city, region } = useTenantContact();
  const whatsappNumber = whatsapp.replace(/\D/g, "") || "";
  const area = [city, region].filter(Boolean).join(", ") || "nuestra zona de cobertura";

  const faqs = useMemo(() => [
    { q: "¿Cuál es el pedido mínimo?", a: "El pedido mínimo puede variar según la configuración de la tienda." },
    { q: "¿Cuánto demora la entrega?", a: `Las entregas se realizan en 24-48 horas hábiles dentro de ${area} y su área de cobertura.` },
    ...
    { q: "¿Hacen entregas fuera de la zona principal?", a: `Por ahora operamos en ${area} y su área de cobertura. Estamos en expansión.` },
  ], [area]);
